...
public class Api16
{
	...
	public static void displayNotification(Context context,
			final Bitmap profileImg, final Status s, final Builder nb,
			final NotificationManager nm)
	{
					...
					new ImageManager.OnImageReceivedListener()
					{
						...
						public void onImageReceived(String arg0, Bitmap media)
						{
							Notification noti = new Notification.BigPictureStyle(
									nb).bigPicture(media)
									.bigLargeIcon(profileImg).build();
							nm.notify(s.getId() + "", Api11.SINGLE_NOTIFCATION,
									noti);
						}
					});
		}
		...
		{
			Notification noti = new Notification.BigTextStyle(nb).bigText(
					s.getText()).build();
