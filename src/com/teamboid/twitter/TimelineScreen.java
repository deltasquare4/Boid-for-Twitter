...
public class TimelineScreen extends Activity
{
	...
	public class SendTweetUpdater extends BroadcastReceiver
	{
		...
		public void onReceive(Context arg0, Intent intent)
		{
			...
			try
			{
				sentTweetBinder = new SendTweetArrayAdapter(
	...
	private void initialize(Bundle savedInstanceState)
	{
		...
		if (!prefs.contains("cab"))
			prefs.edit().putBoolean("cab", true).commit();
	...
	protected void onNewIntent(Intent intent)
	{
		...
		if (intent.getExtras() != null
				&& intent.getExtras().containsKey("new_column"))
		{
			newColumn = true;
			recreate();
		}
	...
	public void loadColumns(boolean firstLoad, boolean accountSwitched)
	{
		...
		final SharedPreferences prefs = PreferenceManager
				.getDefaultSharedPreferences(getApplicationContext());
		ArrayList<String> cols = Utilities.jsonToArray(
				this,
				prefs.getString(
		...
		for (String c : cols)
		{
			if (c.equals(TimelineFragment.ID))
			{
				...
				if (iconic)
				{
					Drawable icon = getTheme().obtainStyledAttributes(new int[]
			...
			else if (c.equals(MentionsFragment.ID))
			{
				...
				{
					Drawable icon = getTheme().obtainStyledAttributes(new int[]
					{ R.attr.mentionsTab }).getDrawable(0);
					toAdd.setIcon(icon);
				}
				else
			...
			else if (c.equals(MessagesFragment.ID))
			{
				...
				{
					Drawable icon = getTheme().obtainStyledAttributes(new int[]
					{ R.attr.messagesTab }).getDrawable(0);
					toAdd.setIcon(icon);
				}
				else
			...
			else if (c.equals(TrendsFragment.ID))
			{
				...
				{
					Drawable icon = getTheme().obtainStyledAttributes(new int[]
					{ R.attr.trendsTab }).getDrawable(0);
					toAdd.setIcon(icon);
				}
				else
			...
			else if (c.equals(FavoritesFragment.ID))
			{
				...
				{
					Drawable icon = getTheme().obtainStyledAttributes(new int[]
					{ R.attr.favoritesTab }).getDrawable(0);
					toAdd.setIcon(icon);
				}
				else
				...
				else
				{
					...
					if (iconic)
					{
						Drawable icon = getTheme().obtainStyledAttributes(
								new int[]
								{ R.attr.savedSearchTab }).getDrawable(0);
						toAdd.setIcon(icon);
			...
			else if (c.startsWith(UserListFragment.ID + "@"))
			{
				...
				{
					Drawable icon = getTheme().obtainStyledAttributes(new int[]
					{ R.attr.userListTab }).getDrawable(0);
					toAdd.setIcon(icon);
			...
			else if (c.equals(NearbyFragment.ID))
			{
				...
				{
					Drawable icon = getTheme().obtainStyledAttributes(new int[]
					{ R.attr.nearbyTab }).getDrawable(0);
					toAdd.setIcon(icon);
				}
				else
	...
	public void recreateAdapters()
	{
		...
		if (AccountService.searchFeedAdapters != null)
		{
			...
			{
				Utilities.recreateSearchAdapter(this,
						AccountService.searchFeedAdapters.get(i));
			}
		}
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		else
		{
			if (getIntent().getExtras() != null
					&& getIntent().getExtras().containsKey("new_column"))
			{
				newColumn = true;
	...
	/**
	 * Called when accounts are loaded on activity load (along with
	 * loadColumns(boolean, boolean))
	 */
			...
			for (int i = 0; i < accs.size(); i++)
			{
				...
				imageManager.get(
						"https://api.twitter.com/1/users/profile_image?screen_name="
								...
								+ "&size=bigger", new OnImageReceivedListener()
						{
							@Override
	...
	private Boolean performRefresh()
	{
		...
		if (frag != null)
		{
			if (frag instanceof NearbyFragment)
				((NearbyFragment) frag).location = null;
	...
	private void removeColumn(int index)
	{
		...
		if (beforeDefCol > 0)
			prefs.edit().putInt(prefName, beforeDefCol - 1).commit();
		ArrayList<String> cols = Utilities.jsonToArray(
				this,
				prefs.getString(
						Long.toString(AccountService.getCurrentAccount()
		...
		switch (item.getItemId())
		{
			case R.id.donateAction:
				Toast.makeText(getApplicationContext(),
						R.string.donations_appreciated, Toast.LENGTH_SHORT)
						.show();
			...
			case R.id.addSavedSearchColAction:
				Toast.makeText(getApplicationContext(),
						getString(R.string.loading_savedsearches),
				...
				new Thread(new Runnable()
				{
					public void run()
					{
						...
						try
						{
							final ResponseList<SavedSearch> lists = acc
									.getClient().getSavedSearches();
							runOnUiThread(new Runnable()
							{
								public void run()
								{
									showSavedSearchColumnAdd(lists
											.toArray(new SavedSearch[0]));
								}
							});
						}
				...
				for (Account acc : AccountService.getAccounts())
				{
							...
							.equals(item.getTitle().toString().substring(1)))
					{
						...
						PreferenceManager
								...
								.putLong("last_sel_account", acc.getId())
								.commit();
						loadColumns(false, true);
						break;
					}
	...
	private void showUserListColumnAdd(final UserList[] lists)
	{
		...
		{
			Toast.makeText(getBaseContext(), getString(R.string.no_lists),
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		builder.setItems(items.toArray(new String[0]),
				new DialogInterface.OnClickListener()
				{
					public void onClick(DialogInterface dialog, int item)
					{
						...
						addColumn(UserListFragment.ID + "@"
								+ curList.getFullName().replace("@", "%40")
								+ "@" + Integer.toString(curList.getId()));
					}
				});
		builder.create().show();
	}
	...
	private void showSavedSearchColumnAdd(final SavedSearch[] lists)
	{
		...
		list.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
					...
					long id)
			{
				...
				addColumn(SavedSearchFragment.ID + "@"
						+ curList.getQuery().replace("@", "%40"));
				diag.dismiss();
			}
		});
		list.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener()
		{
			...
			public boolean onItemLongClick(AdapterView<?> arg0, View arg1,
					int index, long id)
			{
				Toast.makeText(TimelineScreen.this,
						R.string.swipe_to_delete_items, Toast.LENGTH_LONG)
						.show();
				return false;
			}
		});
							...
							catch (Exception e)
							{
								...
								runOnUiThread(new Runnable()
								{
									public void run()
									{
										Toast.makeText(
												getApplicationContext(),
												R.string.savedsearch_upload_error,
												Toast.LENGTH_SHORT).show();
									}
								});
								return;
