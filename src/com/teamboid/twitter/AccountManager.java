...
public class AccountManager extends ListActivity
{
	...
	private void showFollowDialog(final Account acc)
	{
		new Thread(new Runnable()
		{
			public void run()
			{
				try
				{
					final boolean following = acc.getClient().existsFriendship(
							acc.getUser().getScreenName(), "boidapp");
					if (!following)
					{
		...
		listView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener()
		{
					...
					int arg2, long arg3)
			{
				Toast.makeText(AccountManager.this,
						R.string.swipe_to_delete_accounts, Toast.LENGTH_LONG)
						.show();
				return false;
			}
		});
		SwipeDismissListViewTouchListener touchListener = new SwipeDismissListViewTouchListener(
				...
				new SwipeDismissListViewTouchListener.OnDismissCallback()
				{
							...
							int[] reverseSortedPositions)
					{
						...
						{
							AccountService.removeAccount(AccountManager.this,
									(Account) adapter.getItem(i));
							adapter.notifyDataSetChanged();
						}
					}
				});
				...
				if (newValue)
				{
					...
					diag.setPositiveButton(R.string.yes_str,
							new DialogInterface.OnClickListener()
							{
								...
								{
									prefs.edit()
											.putBoolean("enable_ssl", newValue)
											.commit();
									invalidateOptionsMenu();
									int index = 0;
									for (Account acc : AccountService
											.getAccounts())
									{
										...
										final Twitter toAdd = new TwitterFactory(
	...
	private void startAuth()
	{
		...
		AccountService.pendingClient.setOAuthConsumer("5LvP1d0cOmkQleJlbKICtg",
				"j44kDQMIDuZZEvvCHy046HSurt8avLuGeip2QnOpHKI");
		new Thread(new Runnable()
		{
			public void run()
			{
				try
				{
					final RequestToken requestToken = AccountService.pendingClient
							.getOAuthRequestToken("boid://auth");
					startActivityForResult(new Intent(AccountManager.this,
							LoginHandler.class).putExtra("url",
							requestToken.getAuthorizationURL()), 600);
				}
				catch (final TwitterException e)
				{
					e.printStackTrace();
