...
public class MutingManager extends ListActivity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
					...
					else
					{
						switch (typeSpin.getSelectedItemPosition())
						{
							default:
								...
								break;
							case 1:
								if (!term.startsWith("%40"))
									term = ("%40" + term);
								if (adapt.add(term + "@" + types[1]))
									input.setText("");
								break;
							case 2:
								if (adapt.add(term + "@" + types[2]))
									input.setText("");
								break;
						}
					}
		...
		ArrayAdapter<CharSequence> typeAdapt = ArrayAdapter.createFromResource(
				this, R.array.muting_types, R.layout.spinner_item_small);
		typeAdapt
		...
		listView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener()
		{
			...
			public boolean onItemLongClick(AdapterView<?> arg0, View arg1,
					int index, long id)
			{
				Toast.makeText(MutingManager.this,
		...
		if (!prefs.contains("enable_muting"))
			prefs.edit().putBoolean("enable_muting", true).apply();
	}
	...
	public void backup()
	{
		try
		{
			BufferedWriter buf = new BufferedWriter(new FileWriter(new File(
					...
					"Boid_MutingBackup.txt").getAbsolutePath()));
		...
		Toast.makeText(getApplicationContext(), R.string.backed_up_muting,
	...
	public void restore()
	{
		File fi = new File(Environment.getExternalStorageDirectory(),
				"Boid_MutingBackup.txt");
		if (!fi.exists())
		{
			Toast.makeText(getApplicationContext(), R.string.no_muting_backup,
					Toast.LENGTH_SHORT).show();
			return;
		}
		try
		...
		Toast.makeText(getApplicationContext(), R.string.restored_muting,
				Toast.LENGTH_SHORT).show();
	}
}