...
public class Extractor
{
	...
	public List<String> extractSearches(String text)
	{
		if (text == null || text.trim().length() == 0)
			return Collections.emptyList();
		List<String> extracted = new ArrayList<String>();
		for (Entity entity : extractSearchesWithIndices(text))
			extracted.add(entity.value);
		return extracted;
	}
	...
	private List<Entity> extractSearchesWithIndices(String text,
			boolean checkUrlOverlap)
	{
		if (text == null || text.trim().length() == 0)
			return Collections.emptyList();
		...
		while (matcher.find())
		{
			...
			if (!Regex.INVALID_HASHTAG_MATCH_END.matcher(after).find())
			{
				extracted.add(new Entity(matcher, Entity.Type.SEARCH,
						Regex.VALID_HASHTAG_GROUP_TAG));
			}
		}
		if (checkUrlOverlap)
	...
	private static final class IndexConverter
	{
		...
		int codeUnitsToCodePoints(int charIndex)
		{
			if (charIndex < this.charIndex)
				codePointIndex -= text
						.codePointCount(charIndex, this.charIndex);
			else
				codePointIndex += text
						.codePointCount(this.charIndex, charIndex);
