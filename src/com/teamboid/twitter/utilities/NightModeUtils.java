...
public class NightModeUtils {
	...
	public static boolean isNightMode(Context c){
		...
		if( prefs.getBoolean("night_mode", false) == false ) return false; // SLEEP IS FOR THE WEAK!
		
		Calendar now = Calendar.getInstance();
		Calendar time = parseTimePreference(prefs.getString("night_mode", "00:00"));
		if(now.after(time)){ // We have passed the begining of the night time
			time = parseTimePreference(prefs.getString("night_mode_endtime", "00:00"));
