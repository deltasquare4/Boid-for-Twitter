...
public class Utilities
{
	...
	public static void recreateSearchAdapter(Activity context,
			SearchFeedListAdapter adapt)
	{
		Tweet[] before = null;
		if (adapt.getCount() > 0)
			before = adapt.toArray();
				...
				&& tweet.getURLEntities().length > 0)
		{
			...
			for (int i = 0; i < tweet.getURLEntities().length; i++)
			{
				...
				if (urlEntity.getExpandedURL() != null)
					curEntity = urlEntity.getExpandedURL().toString();
				if (curEntity.endsWith(".jpg") || curEntity.endsWith(".jpeg")
						|| curEntity.endsWith(".png")
						|| curEntity.endsWith(".bmp")
						|| curEntity.endsWith(".gif")
						|| curEntity.endsWith(".bmp"))
				{
					found = curEntity;
					break;
			...
			final boolean expand)
	{
		if (urls != null)
		{
			for (URLEntity url : urls)
			{
				...
				{
					text = text.replace(url.getURL().toString(), url
							.getExpandedURL().toString());
				}
				else if (url.getDisplayURL() != null)
					text = text.replace(url.getURL().toString(),
							url.getDisplayURL());
			}
		}
		if (pics != null)
	...
	public static String friendlyTimeMedium(Date createdAt)
	{
		...
		else if (diff <= 604800000)
		{
			...
			return Long.toString(diff / 604800000) + " weeks";
	}
	...
	public static String friendlyTimeLong(Context context, Date createdAt)
	{
		...
		if (now.get(Calendar.YEAR) == time.get(Calendar.YEAR))
		{
			...
			{
				return Integer.toString(time.get(Calendar.HOUR)) + ":" + minute
						+ am_pm + " "
						+ convertMonth(time.get(Calendar.MONTH), false) + " "
						+ day;
			}
	...
	{
		return getAllMentions(tweet.getFromUser(),
	...
	public static Bitmap getRoundedImage(Bitmap bitmap, float roundPx)
	{
		...
		// TODO recycling causes crash because "recycled images cannot be used"
		// or something
		// bitmap.recycle();
		return output;
	}
			...
			twitter4j.User user)
	{
		String url = "https://api.twitter.com/1/users/profile_image?screen_name="
				+ screenname;
		if (user != null)
		{ // Allows us to have auto-updating cache
