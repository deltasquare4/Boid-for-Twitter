...
public class PushReceiver extends BroadcastReceiver
{
		...
		public int onStartCommand(final Intent intent, int flags, int startId)
		{
					...
					public void run()
					{
						try
						{
							...
							final URL url = new URL(SERVER
									+ "/register?userid=" + acc.getId()
									+ "&token="
									+ Uri.encode(intent.getStringExtra("reg"))
									+ "&accesstoken="
									+ Uri.encode(acc.getToken())
									+ "&accesssecret="
									+ Uri.encode(acc.getSecret()));

							HttpsURLConnection urlConnection = (HttpsURLConnection) url
									.openConnection();
							urlConnection
									.setSSLSocketFactory(SSLCertificateSocketFactory
											.getInsecure(3000, null));
							urlConnection
			...
			else if (intent.hasExtra("hm"))
			{
				...
				try
				{
					...
					final twitter4j.Status s = new twitter4j.internal.json.StatusJSONImpl(
							status);
					// TODO The account the mention is for should be passed from
					// the server too
					// We have this now in "account" as a string
					// Also, we need a way of combining multiple
					// mentions/messages into one notification.
	...
	private void handleMessage(Context context, Intent intent)
	{
		...
		context.startService(new Intent(context, PushWorker.class).putExtra(
				"hm", intent.getExtras()));
	}
	...
	private void handleRegistration(Context context, Intent intent)
	{
		...
		else if (registration != null)
		{
			...
			context.startService(new Intent(context, PushWorker.class)
					.putExtra("reg", registration));
		}
	}
