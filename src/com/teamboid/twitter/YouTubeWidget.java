...
public class YouTubeWidget extends TweetWidget
{
	...
	{
		Log.d("yt", "Found me @ " + intent.getData().toString() + " "
		...
		try
		{
			...
			HttpResponse r = dfhc.execute(new HttpGet(
					"http://gdata.youtube.com/feeds/api/videos/" + vidid
							+ "?v=2&alt=json&&fields=title"));
			if (r.getStatusLine().getStatusCode() == 200)
			{
				JSONObject jo = new JSONObject(EntityUtils.toString(
						r.getEntity(), "UTF-8")).getJSONObject("entry");
		...
		final RemoteViews rv = new RemoteViews("com.teamboid.twitter",
				R.layout.youtube_widget);
		...
		PendingIntent pendingIntent = PendingIntent.getActivity(cntxt, 0, send,
				0);
		...
		rv.setBitmap(
				R.id.thumb,
				"setImageBitmap",
				imageManager.get("http://i.ytimg.com/vi/" + vidid
						+ "/mqdefault.jpg"));
