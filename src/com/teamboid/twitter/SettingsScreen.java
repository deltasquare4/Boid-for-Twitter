...
public class SettingsScreen extends PreferenceActivity
{
	...
	public static class HelpFragment extends PreferenceFragment
	{
		...
		public void onCreate(Bundle savedInstanceState)
		{
			...
			((Preference) findPreference("version")).setSummary(Utilities
					.getVersionName(getActivity()));
			((Preference) findPreference("build")).setSummary(Utilities
					.getVersionCode(getActivity()));
			((Preference) findPreference("share_boid"))
							...
							startActivity(Intent
									.createChooser(
											new Intent(Intent.ACTION_SEND)
													...
													.putExtra(
															Intent.EXTRA_TEXT,
															getString(R.string.share_boid_content)),
											getString(R.string.share_boid_title)));
							return false;
						}
					});
			((Preference) findPreference("boidapp"))
					.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener()
					{
						@Override
						public boolean onPreferenceClick(Preference preference)
						{
							startActivity(new Intent(getActivity(),
									ProfileScreen.class).addFlags(
									Intent.FLAG_ACTIVITY_CLEAR_TOP).putExtra(
									"screen_name", "boidapp"));
							return false;
						}
					});
			((Preference) findPreference("donate"))
					.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener()
					{
						@Override
						public boolean onPreferenceClick(Preference preference)
						{
							BillingController.requestPurchase(getActivity(),
	...
	public static class GeneralFragment extends PreferenceFragment
	{
		...
		public void onCreate(Bundle savedInstanceState)
		{
			...
			((SwitchPreference) findPreference("enable_ssl"))
								...
								Preference preference, Object newValue)
						{
							...
							for (Account acc : AccountService.getAccounts())
							{
								ConfigurationBuilder cb = new ConfigurationBuilder()
										.setOAuthConsumerKey(
												"5LvP1d0cOmkQleJlbKICtg")
										.setOAuthConsumerSecret(
												"j44kDQMIDuZZEvvCHy046HSurt8avLuGeip2QnOpHKI")
										.setOAuthAccessToken(acc.getToken())
										.setOAuthAccessTokenSecret(
												acc.getSecret())
										.setUseSSL((Boolean) newValue)
										.setJSONStoreEnabled(true);
								final Twitter toAdd = new TwitterFactory(cb
										.build()).getInstance();
								acc.setClient(toAdd);
	...
	public static class ComposerFragment extends PreferenceFragment
	{
		private void setMediaName()
		{
			...
			String pref = uploadService.getSharedPreferences().getString(
					"upload_service", "twitter");
			try
			{
				uploadService.setSummary(MediaUtilities.getMediaServices(true,
						getActivity()).get(pref).name);
			}
			catch (Exception e)
			{
				e.printStackTrace();
				...
				Intent data)
		{
			if (requestCode == SELECT_MEDIA && resultCode == RESULT_OK)
			{
				PreferenceManager
						...
						.edit()
						.putString("upload_service",
								data.getStringExtra("service")).commit();
				setMediaName();
			}
		}
	...
	public static class AppearanceFragment extends PreferenceFragment
	{
		...
		public void onCreate(Bundle savedInstanceState)
		{
			...
			((ListPreference) findPreference("boid_theme"))
			...
			findPreference("font_size").setOnPreferenceClickListener(
		...
		private void showFontDialog()
		{
			...
			final TextView display = (TextView) diag
					.findViewById(R.id.fontSizeDialogExample);
			final NumberPicker picker = (NumberPicker) diag
					.findViewById(R.id.fontSizePicker);
			picker.setOnValueChangedListener(new NumberPicker.OnValueChangeListener()
			{
				...
				public void onValueChange(NumberPicker picker, int oldVal,
						int newVal)
				{
					display.setText(getString(R.string.boid_is_awesome) + " "
							+ newVal + "pt");
					display.setTextSize(newVal);
					prefs.edit()
							.putString("font_size", Integer.toString(newVal))
							.apply();
				}
			});
	...
	public static class NotificationsFragment extends PreferenceFragment
	{
		...
		public void onCreate(Bundle savedInstanceState)
		{
			...
			((SwitchPreference) findPreference("c2dm"))
					.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener()
					{
						@Override
						public boolean onPreferenceChange(
								...
								registrationIntent.putExtra("app",
										PendingIntent.getBroadcast(
												getActivity(), 0, new Intent(
														getActivity(),
														PushReceiver.class), 0)); // boilerplate
