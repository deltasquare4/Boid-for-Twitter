...
public class UserListDisplayAdapter extends BaseAdapter
{
	...
	public boolean contains(UserList list)
	{
		...
		{
			if (lists.get(i).getId() == list.getId())
				return true;
		}
		return false;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		if (convertView != null)
			...
			toReturn = (TextView) LayoutInflater.from(mContext).inflate(
					R.layout.trends_list_item, null);
	...
	public void destroyOrUnsubscribe(final int pos)
	{
		...
		if (list.getUser().getId() == AccountService.getCurrentAccount()
				.getId())
		{
			AlertDialog.Builder diag = new AlertDialog.Builder(mContext);
			diag.setTitle(R.string.delete_str);
			diag.setMessage(mContext.getString(R.string.delete_list_prompt)
					.replace("{listname}", list.getFullName()));
		...
		else
		{
			...
			diag.setMessage(mContext
					.getString(R.string.unsubscribe_list_prompt).replace(
							"{listname}", list.getFullName()));
			diag.setPositiveButton(R.string.yes_str,
					new DialogInterface.OnClickListener()
					{
						@Override
						...
						{
							final Toast toast = Toast.makeText(mContext,
									R.string.unsubscribing_list_str,
									Toast.LENGTH_LONG);
							toast.show();
							new Thread(new Runnable()
							{
								public void run()
								{
									...
									mContext.runOnUiThread(new Runnable()
									{
										public void run()
										{
											...
											Toast.makeText(
													mContext,
													R.string.unsubscribed_list_str,
													Toast.LENGTH_SHORT).show();
											remove(list.getId());
										}
									});
