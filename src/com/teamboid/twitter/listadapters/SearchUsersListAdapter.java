...
public class SearchUsersListAdapter extends BaseAdapter
{
	...
	{
		return users.toArray(new Tweet[0]);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		final RemoteImageView profilePic = (RemoteImageView) toReturn
				.findViewById(R.id.userItemProfilePic);
		profilePic.setImageResource(R.drawable.sillouette);
		profilePic.setImageURL(Utilities.getUserImage(user.getScreenName(),
				mContext));
		TextView userName = (TextView) toReturn.findViewById(R.id.userItemName);
		FeedListAdapter.ApplyFontSize(userName, mContext);
		TextView userDesc = (TextView) toReturn
				.findViewById(R.id.userItemDescription);
		FeedListAdapter.ApplyFontSize(userDesc, mContext);
		userName.setText(user.getName());
		if (user.getDescription() != null
				&& !user.getDescription().trim().isEmpty())
		{
			userDesc.setText(Utilities.twitterifyText(mContext, user
					.getDescription().replace("\n", " ").trim(), null, null,
					false));
		}
		else
			userDesc.setText(mContext.getApplicationContext().getString(
					R.string.nodescription_str));
		if (user.isVerified())
			((ImageView) toReturn.findViewById(R.id.userItemVerified))
