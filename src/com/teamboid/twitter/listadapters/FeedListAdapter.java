...
public class FeedListAdapter extends BaseAdapter
{
	...
	public Status getTweet(int at)
	{
		return tweets.get(at);
	}
	...
	private boolean shouldFilter(Status tweet, String query, String type)
	{
		...
		{
			if (Html.fromHtml(tweet.getSource()).toString().toLowerCase()
	...
	public int add(Status[] toAdd, boolean filter)
	{
		...
		String[] fi = null;
		if (filter)
			fi = Utilities.jsonToArray(mContext, prefs.getString(prefName, ""))
	...
	public View getView(final int position, View convertView, ViewGroup parent)
	{
		...
		View replyFrame = (RelativeLayout) toReturn
				.findViewById(R.id.inReplyToFrame);
		...
		if (tweet.isRetweet())
		{
			Spannable rtSpan = new SpannableString("RT by @"
		...
		if (tweet.getGeoLocation() != null || tweet.getPlace() != null)
		{
			...
			else
			{
				...
				locIndicator.setText(Double.toString(g.getLatitude()) + ", "
						+ Double.toString(g.getLongitude()));
			}
		}
		else
