...
public class ProfileAboutAdapter extends BaseAdapter {
	...
	public void setIsError(final boolean error) {
		mContext.runOnUiThread(new Runnable() {
			public void run() {
				isError = error;
				isUnknown = (!isBlocked && !isFollowing && !isFollowedBy
						&& !isRequestSent && !isError);
	...
	public void setUser(User _user) {
		...
		if (!desc.isEmpty()) {
			values.add(new BasicNameValuePair(mContext
					.getString(R.string.bio_str), desc));
		}
		values.add(new BasicNameValuePair(mContext
				.getString(R.string.tweets_str), df.format(_user
				.getStatusCount())));
		double tweetsPerDay = user.getStatusCount();
		int days = Days.daysBetween(new DateTime(user.getCreatedAt()),
		...
		values.add(new BasicNameValuePair(mContext
				.getString(R.string.tweets_per_day), Double
				.toString(tweetsPerDay)));
		if (_user.getUrl() != null) {
			values.add(new BasicNameValuePair(mContext
					.getString(R.string.website_str), _user.getUrl().toString()));
		}
		if (_user.getLocation() != null && !_user.getLocation().isEmpty()) {
			values.add(new BasicNameValuePair(mContext
					.getString(R.string.location_str), _user.getLocation()));
		}
		values.add(new BasicNameValuePair(mContext
				.getString(R.string.favorites_str), df.format(_user
				.getFavoritesCount())));
		values.add(new BasicNameValuePair(mContext
				.getString(R.string.friends_str), df.format(_user
				.getFriendsCount())));
		values.add(new BasicNameValuePair(mContext
				.getString(R.string.followers_str), df.format(_user
				.getFollowersCount())));
		notifyDataSetChanged();
	}
	...
	public int getCount() {
		if (user == null)
			return 0;
		if (AccountService.getCurrentAccount().getUser().getId() == user
				.getId()) {
			return values.size();
		} else
	...
	public Object getItem(int position) {
		if (position == 0)
			...
			return values.get(position - 1).getValue();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position == 0
				&& AccountService.getCurrentAccount().getUser().getId() != user
						.getId()) {
			toReturn = (RelativeLayout) LayoutInflater.from(mContext).inflate(
					R.layout.profile_info_tab, null);
			final Button followBtn = (Button) toReturn
					.findViewById(R.id.profileFollowBtn);
			if (isUnknown) {
				...
				if (!isFollowing) {
					if (isFollowedBy)
						followBtn.setText(R.string.follow_back_str);
					else
						followBtn.setText(R.string.follow_str);
				} else {
					if (isFollowedBy)
						followBtn.setText(R.string.follows_you_unfollow_str);
					else
			...
			if (curItem.getName().equals("[VERIFIED]")) {
				...
				return LayoutInflater.from(mContext).inflate(
						R.layout.verified_imageview, null);
			}

			toReturn = (RelativeLayout) LayoutInflater.from(mContext).inflate(
					R.layout.info_list_item, null);
			if (position == 0) {
				// If the position is 0 here that means the follow button isn't
				// visible
			...
			TextView title = (TextView) toReturn
					.findViewById(R.id.infoListItemTitle);
			title.setText(curItem.getName());
			FeedListAdapter.ApplyFontSize(title, mContext, true);
			TextView body = (TextView) toReturn
					.findViewById(R.id.infoListItemBody);
		...
		new Thread(new Runnable() {
			public void run() {
				if (isBlocked) {
					...
					AutocompleteService.removeItem(user, mContext,
					...
					AutocompleteService.addItem(user, mContext, AccountService
