...
/**
 * The list adapter used in the account manager, displays the accounts
 * currenetly added in the AccountService.
 * 
...
public class AccountListAdapter extends BaseAdapter
{
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		RemoteImageView profilePic = (RemoteImageView) toReturn
				.findViewById(R.id.accountItemProfilePic);
		profilePic.setImageResource(R.drawable.sillouette);
		profilePic.setImageURL(Utilities.getUserImage(curUser.getScreenName(),
				mContext));
		TextView nameTxt = (TextView) toReturn
				.findViewById(R.id.accountItemName);
		nameTxt.setText(curUser.getName());
		FeedListAdapter.ApplyFontSize(nameTxt, mContext);
		TextView descTxt = (TextView) toReturn
				.findViewById(R.id.accountItemDescription);
		if (curUser.getDescription() != null
				&& !curUser.getDescription().trim().isEmpty())
		{
			descTxt.setText(Utilities.twitterifyText(mContext,
					curUser.getDescription(), null, null, false));
		}
		else
			descTxt.setText(mContext.getApplicationContext().getString(
					R.string.nodescription_str));
