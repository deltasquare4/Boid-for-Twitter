...
/**
 * The list adapter used in the muting manager, displays the all current muting
 * rules.
 * 
...
public class MutingListAdapter extends BaseAdapter
{
	...
	public int getCount()
	{
		return loadKeywords().length;
	}

	@Override
	public Object getItem(int position)
	{
		return loadKeywords()[position];
	}
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		if (convertView != null)
			...
			toReturn = (RelativeLayout) LayoutInflater.from(mContext).inflate(
					R.layout.muting_manager_item, null);
		final String[] types = mContext.getResources().getStringArray(
		...
		if (curRule.contains("@"))
		{
			text1.setText(curRule.substring(0, curRule.indexOf("@")).replace(
					"%40", "@"));
			if (curRule.endsWith("@" + types[1]))
			{
				...
				text2.setText(types[2].toLowerCase());
		}
		else
		{
	...
	public String[] loadKeywords()
	{
		...
		String prefName = Long.toString(AccountService.getCurrentAccount()
				.getId()) + "_muting";
		return Utilities.jsonToArray(mContext, prefs.getString(prefName, ""))
	...
	public void restorePreference(String[] keywords)
	{
		...
		String prefName = Long.toString(AccountService.getCurrentAccount()
				.getId()) + "_muting";
		ArrayList<String> cols = Utilities.jsonToArray(mContext,
		...
		prefs.edit().putString(prefName, Utilities.arrayToJson(mContext, cols))
				.commit();
	...
	public boolean add(String term)
	{
		...
		if (!cols.contains(term))
		{
			cols.add(term);
			prefs.edit()
					.putString(prefName, Utilities.arrayToJson(mContext, cols))
					.commit();
			notifyDataSetChanged();
			return true;
		}
