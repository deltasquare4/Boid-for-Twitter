...
public class SendTweetArrayAdapter extends ArrayAdapter<SendTweetTask>
{
	...
	public View getView(final int position, View convertView, ViewGroup parent)
	{
		if (convertView == null)
		{
			convertView = ((LayoutInflater) this.getContext().getSystemService(
					Context.LAYOUT_INFLATER_SERVICE)).inflate(
					R.layout.send_tweet, null);
		}
		...
		convertView.findViewById(R.id.feedItemMediaIndicator).setVisibility(
				stt.hasMedia() ? View.VISIBLE : View.GONE);
		RemoteImageView profilePic = (RemoteImageView) convertView
				.findViewById(R.id.feedItemProfilePic);
		profilePic
				.setImageURL("https://api.twitter.com/1/users/profile_image?screen_name="
						+ stt.from.getUser() + "&size=bigger");
		convertView.findViewById(R.id.progressBar).setVisibility(
				stt.result.errorCode == Result.WAITING ? View.VISIBLE
						: View.GONE);
		ImageButton btn = (ImageButton) convertView.findViewById(R.id.delete);
		btn.setVisibility(stt.result.errorCode == Result.WAITING ? View.GONE
				: View.VISIBLE);
