	...
	public SendTweetTask.Result sendTweet(Context context) {
		...
		if (this.hasMedia()) {
			try {
				...
				switch (ems.getNeededAuthorization()) { // We need to send
														// whatever auth it
														// requires
				...
				case OAUTH:
					sp = PreferenceManager.getDefaultSharedPreferences(context);
					Token token = new Token(sp.getString(prefValue + "-token",
							""), sp.getString(prefValue + "-secret", ""));
					ems.setAuthorized(
							MediaUtilities.buildAuthService(prefValue), token);
					break;
				default:
					break;
				}
